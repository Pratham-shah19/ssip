...
const modifyDish = async (req,res)=>{
  ...
  const dish = await Dish.findOneAndUpdate({_id:did},req.body,{new:true,runValidators:true})
  if(!dish){
    throw new NotFoundError('Please provide valid dish id')
  }
  res.status(StatusCodes.OK).json({res:'Success',data:dish})
...
const updateCanteenDetails = async (req,res) => {
  ...
  const canteen = await Canteen.findOneAndUpdate({_id:canteenId},req.body,{new:true,runValidators:true})
  if(!canteen){
...
const guestCompletedButton = async (req,res) => {
  ...
  const completedButton = await Order.findOneAndUpdate({_id:oid},{status:"COMPLETED"},{new:true,runValidators:true})
  if(!completedButton){
...
const modifyQuantity = async (req,res) => {
  ...
  const updatedDish = await Dish.findOneAndUpdate({_id:did},{quantity:newQuantity,isAvailable:true},{new:true,runValidators:true})
...
module.exports = {deletebtn,resetWallet,addDish,modifyQuantity,getCurrentOrders,getHistoryOrders,getUser,getAllDish,getDish,modifyDish,fetchAllCanteen,getUserByEmail,addOrder,updateCanteenDetails,guestCompletedButton,dishFilter,dishCategory}